...
extern char * zafProcL;
extern char * zafProcH;
#else
...
UI_WINDOW_OBJECT *UIW_WINDOW::Add(UI_WINDOW_OBJECT *object)
{
        ...
        else if (Inherited(ID_MENU) && !screenID && parent && parent->screenID)
        {
                ...
                char *mappedStr = MapText(text, ZIL_NULLP(char), FALSE);
...
//-----------------------------------
                ...
                case WM_HSCROLL:
                        {
                        ZIL_SCREENID scrollBar = (ZIL_SCREENID)HIWORD(lParam);
                        int nPos = LOWORD(lParam);
                        int scrollCode = wParam;
                        if (!scrollBar)
                        {
        ...
        if (!processed)
        {
                ...
                case S_CREATE:
                        {
                        char clientString[] = { 'C','L','I','E','N','T', 0 };
                        char clientTitle[] = { 0 };
                        clipList.Destroy();
                        ...
                        {
                                RegisterObject("UIW_MDIFRAME", ZIL_NULLP(char), &_mdiFrameOffset,
                        ...
                        {
                                woStatus |= WOS_WINDOWS_ACTION;
                                RegisterObject("UIW_MDICHILD", ZIL_NULLP(char), &_mdiChildOffset,
                        ...
                        {
                                RegisterObject("UIW_WINDOW", ZIL_NULLP(char), &_windowOffset,
                ...
                case S_REGISTER_OBJECT:
                        ...
                        else if (FlagSet(woAdvancedFlags, WOAF_MDI_OBJECT))
                        {
                                RegisterObject("UIW_MDICHILD", ZIL_NULLP(char), &_mdiChildOffset,
                                        &_mdiChildJumpInstance, &_mdiChildCallback,
                                        title ? (ZIL_ICHAR *)title->Information(I_GET_TEXT, ZIL_NULLP(void)) : ZIL_NULLP(ZIL_ICHAR), menu);
//                              defaultCallback = _mdiChildCallback;
                        }
                        else
                                RegisterObject("UIW_WINDOW", ZIL_NULLP(char), &_windowOffset,
                                        &_windowJumpInstance, &_windowCallback,
