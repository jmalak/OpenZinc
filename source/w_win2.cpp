...
public:
        ...
        MAKEPROC_ELEMENT(char *_name, FARPROC _procInstance, HINSTANCE _hInstance) :
...
int ZincClassAdd(char *name, FARPROC farproc, HINSTANCE hInstance)
{
...
char *zafProcL = "ZAF_PROCL";
char *zafProcH = "ZAF_PROCH";
#else
ATOM zafProcL;
ATOM zafProcH;
#endif
...
//                                              if (oldBrush != ctlHBrush)
        ...
        if (!processed)
        {
                ...
                case S_REGISTER_OBJECT:
                        ...
                        RegisterObject("UI_WINDOW_OBJECT", "BUTTON", &_objectOffset,
                ...
                case E_KEY:
                        ...
                        {
                        char *tmp = MapChar(event.key.value);
                        if (tmp[1])
                                PostMessage(hwnd, WM_CHAR, tmp[1], 0);
                        ccode = CallWindowProc((WINDOWSPROC)defaultCallback, hwnd, WM_CHAR, tmp[0], 0);
                        }
#else
...
#       define _Z_MDIWIN Z_MDIWIN
        ...
        {
#if !defined(__WATCOMC__)
                zafProcL = GlobalAddAtom("ZAFPROCL");
                zafProcH = GlobalAddAtom("ZAFPROCH");
#endif
                ...
                _jumpInstance = MakeProcInstance((FARPROC)JumpProcedure, display->hInstance);
                ZincClassAdd(Z_WIN, _jumpInstance, display->hInstance);
                registeredClass = 1;
...
#       endif
        ...
        extern int ZincClassAdd(char *name, FARPROC farproc, HINSTANCE hInstance);
...
//              ZIL_SCREENID parentId = IsDialog() ? windowManager->taskID : 0;
                ...
                screenID = CreateWindowEx(exFlags, baseName, TEXT_, dwStyle & ~(WS_VSCROLL | WS_HSCROLL),
                        trueRegion.left, trueRegion.top, trueRegion.Width(), trueRegion.Height(),
                        parentId, menu, display->hInstance, ZIL_NULLP(ZIL_ICHAR));
//                      windowManager->taskID, menu, display->hInstance, ZIL_NULLP(ZIL_ICHAR));
                ...
                defaultCallback = (FARPROC)SetWindowLong(screenID, GWL_WNDPROC, (LONG)JumpProcedure);
        ...
        else if (parent->Inherited(ID_COMBO_BOX))
        {
                ...
                DWORD index = SendMessage(screenID, CB_INSERTSTRING,
                        ...
                        TEXT_ ? (LPARAM)TEXT_ : (LPARAM)_blankString);
        ...
        else if (FlagSet(woAdvancedFlags, WOAF_MDI_OBJECT))
        {
                ...
                mdiCreate.szTitle = TEXT_;
                mdiCreate.hOwner = display->hInstance;
                mdiCreate.style = dwStyle & ~(WS_VSCROLL | WS_HSCROLL);
                ...
                mdiCreate.lParam = 0;
                screenID = (HWND)LOWORD(SendMessage(parent->screenID, WM_MDICREATE, 0, (LPARAM)&mdiCreate));
                SetWindowLong(screenID, GWL_STYLE, (dwStyle & ~WS_POPUP) | WS_CAPTION | WS_CLIPSIBLINGS);
        ...
        else
        {
                ...
                HINSTANCE hInstance = Inherited(ID_TEXT) && menu ?
                        (HINSTANCE)menu : display->hInstance;

                FARPROC jumpProcedure;
                if (hInstance != display->hInstance)
                {
                        jumpProcedure = MakeProcInstance((FARPROC)JumpProcedure, hInstance);
                        ZincClassAdd(baseName, jumpProcedure, hInstance);
                }
                else
                        jumpProcedure = _jumpInstance;


                screenID = CreateWindowEx(exFlags, baseName, TEXT_, dwStyle,
                        trueRegion.left, trueRegion.top, trueRegion.Width(), trueRegion.Height(),
                        parentID, (HMENU)numberID, hInstance, ZIL_NULLP(ZIL_ICHAR));
...
{
                dwStyle = 0;
                defaultCallback = ZIL_NULLF(FARPROC);
}
