...
LRESULT FAR PASCAL _export ScrollJumpProcedure(HWND hWnd, WORD wMsg, WPARAM wParam, LRESULT lParam)
{
...
EVENT_TYPE UIW_SCROLL_BAR::Event(const UI_EVENT &event)
{
	...
	if (event.type == E_MSWINDOWS)
	{
		...
		case WM_VSCROLL:
			...
			WORD notification = event.message.wParam;
			switch (notification)
			{
			case SB_BOTTOM:
			...
			case SB_THUMBTRACK:
				scroll.current = LOWORD(event.message.lParam) * scroll.delta;
				if (FlagsSet(sbFlags, SBF_SLIDER | SBF_VERTICAL))
			...
			if (tCurrent != scroll.current)
				if (FlagsSet(sbFlags, SBF_SLIDER | SBF_VERTICAL))
					SetScrollPos(screenID, SB_CTL, scroll.maximum + scroll.minimum - scroll.current / scroll.delta, TRUE);
				else
	...
	if (!processed)
	{
		...
		case S_CREATE:
			...
			if (FlagSet(sbFlags, SBF_CORNER | SBF_HORIZONTAL))
				if (FlagSet(woFlags, WOF_NON_FIELD_REGION))
				{
					relative.top = relative.bottom - GetSystemMetrics(SM_CXVSCROLL);
					trueRegion.top = trueRegion.bottom + relative.Height() - 1;
				}
				else
				{
					relative.bottom = relative.top + GetSystemMetrics(SM_CXVSCROLL);
		...
		case S_HSCROLL:
			...
			if (FlagsSet(sbFlags, SBF_SLIDER | SBF_VERTICAL))
				SetScrollPos(windowID, scrollID, scroll.maximum + scroll.minimum - scroll.current / scroll.delta, TRUE);
			else
